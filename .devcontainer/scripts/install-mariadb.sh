...
if [ -d /workspaces/devcontainer-php-mysql/db-data/mysql ]; then
	echo "[i] MySQL directory already present, skipping creation"
	chown -R mysql:mysql /workspaces/devcontainer-php-mysql/db-data/mysql
else
	echo "[i] MySQL data directory not found, creating initial DBs"
	mkdir /workspaces/devcontainer-php-mysql/db-data/mysql
	chown -R mysql:mysql /workspaces/devcontainer-php-mysql/db-data/mysql

	mysql_install_db --user=mysql --ldata=/workspaces/devcontainer-php-mysql/db-data/mysql > /dev/null
...
EOF
	...
	/usr/bin/mysqld --datadir=/workspaces/devcontainer-php-mysql/db-data/mysql --user=mysql --bootstrap --verbose=0 --skip-name-resolve --skip-networking=0 < $tfile
	rm -f $tfile
	...
	if [ "$MYSQL_DATABASE" != "" ] && [ "$(ls -A /docker-entrypoint-initdb.d 2>/dev/null)" ]; then
		...
		/usr/bin/mysqld --datadir=/workspaces/devcontainer-php-mysql/db-data/mysql --user=mysql --skip-name-resolve --skip-networking=0 --silent-startup > "${TEMP_OUTPUT_LOG}" 2>&1 &
		PID="$!"
	...
	echo "exec /usr/bin/mysqld --datadir=/workspaces/devcontainer-php-mysql/db-data/mysql --user=mysql --console --skip-name-resolve --skip-networking=0" "$@"
fi

exec /usr/bin/mysqld --datadir=/workspaces/devcontainer-php-mysql/db-data/mysql --user=mysql --console --skip-name-resolve --skip-networking=0 $@